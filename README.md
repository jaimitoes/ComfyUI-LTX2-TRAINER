...
* About versions : The code has many changes giving extra optimizations compared to the official trainer, improving Dynamo Inductor and memory management. I noticed this days fewer updates from Lightricks in their packages. As the current version is working great, i will give a time to add the newest Lightricks implementations (rebuilding the code takes time and the LightTricks commits does not stop during the early stages). Whatever, the last updates from Lightricks seems to be related with inference. This build works perfectly creating amazing LoRAs in single gpu (not tested in multi-gpu, but you can always post an issue in case).
