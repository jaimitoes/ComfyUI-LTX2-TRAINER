...
* Custom nodes changes between updates, in the moment of pushing this, new options as setted up qwen vl nodes (Also there is different versions with the same name and can create conflicts) If you still in troubles with it you also have the QwenOmni node Used by LTX-2, make sure to enter into the subgraph and check everything again (subgraphs are a bit dirty in that aspect).
